...
def legacy_era5_spectra_to_snapwave_timeseries(
    ds: xr.Dataset,
    points: gpd.GeoDataFrame,
    event_window: tuple,
    variables: dict[str, str] | None = None,
) -> dict[str, pd.DataFrame]:
    """Return the legacy ``sfincs_runs`` SnapWave table dict from ERA5 spectra."""
    variables = variables or ERA5_SNAPWAVE_MAP
    start, stop = event_window
    time_name = _time_coord(ds)
    window = ds.sel({time_name: slice(start, stop)})
    result: dict[str, pd.DataFrame] = {}
    points_wgs84 = points.set_crs("EPSG:4326") if points.crs is None else points.to_crs("EPSG:4326")
    for key, era5_name in variables.items():
        per_point = {}
        for _, row in points_wgs84.iterrows():
            per_point[row["name"]] = _nearest_finite_series(
                window,
                era5_name,
        ...
        result[key] = pd.DataFrame(per_point, index=window[time_name].values)
    return result
