...
def cn_recovery_seff(smax, soil_saturation, *, units="fraction"):
    """Initial effective storage for SFINCS CN-with-recovery from top-layer saturation."""
...
def prepare_aorc_precip_for_sfincs(
    ...
    *,
    ...
    """Write HydroMT-SFINCS-ready interval precipitation as ``precip(time, y, x)``.

    AORC APCP is mm over the reporting interval. ``scale_factor`` multiplies the
    whole spatio-temporal field by ``K = target / observed`` so a sampled rainfall
    design target is realized by scaling the observed AORC SST field rather than
    collapsing it to a scalar (the Field-Preserving Realization).
    """
    scale_factor = float(scale_factor)
    if not np.isfinite(scale_factor) or scale_factor <= 0:
        raise ValueError(f"scale_factor must be finite and positive, got {scale_factor!r}")
    source_nc, output_nc = Path(source_nc), Path(output_nc)
    ...
    with xr.open_dataset(source_nc) as ds:
        ...
        da = ds[variable].rename({k: v for k, v in {"latitude": "y", "longitude": "x"}.items() if k in ds[variable].dims})
        da = da.rename("precip")
        ...
        # Always anchor the window to a start time: precip_start when the catalog
        # gives one, else the run's own t_start (HydroMT-SFINCS needs precip on
        # the run's own clock, not the source file's storm-window timestamps).
        ...
        precip = (da.sortby("y").sortby("x").reindex(time=full_time, fill_value=0.0) * scale_factor).astype("float32")
        precip.attrs.update(units="mm", crs="EPSG:4326", applied_scale_factor=scale_factor)
