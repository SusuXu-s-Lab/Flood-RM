...
from collect_sources.era5 import CDS_WAVE_VARIABLES, cds_payload

era5_wave_variables = CDS_WAVE_VARIABLES


def build_cds_request_payload(bbox_wgs84, time_window, variables=None) -> dict:
    return cds_payload(bbox_wgs84, time_window[0], time_window[1], variables)
...
def fetch_era5_waves(bbox_wgs84, time_window, output_path, variables=None, force=False) -> Path:
    ...
    try:
        ...
        raise RuntimeError("cdsapi not installed. Run: uv add cdsapi (or pip install cdsapi).") from exc
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Fetch ERA5 ocean wave reanalysis from CDS.")
    parser.add_argument("--bbox", required=True, nargs=4, type=float, metavar=("W", "S", "E", "N"))
    parser.add_argument("--start", required=True, type=pd.Timestamp, help="event window start (ISO)")
    parser.add_argument("--stop", required=True, type=pd.Timestamp, help="event window stop (ISO)")
    parser.add_argument("--out", required=True, type=Path, help="output NetCDF path")
