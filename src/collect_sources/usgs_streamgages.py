...
from collect_sources.audit import (
    Artifact as AuditArtifact,
    covers as audit_covers,
    resolve as audit_resolve,
    write_artifact as audit_write_artifact,
)
...
# Compact registry collector (formerly collect_sources.usgs). Writes audit-style
# artifacts for the "usgs" collection key; the notebook-facing streamgage review
# workflow above uses source_artifacts manifests instead.
...
def collect(settings: dict, *, skip_existing=False) -> AuditArtifact:
    paths, spec = settings["paths"], settings["spec"]
    start, end = pd.Timestamp(settings["start"]), pd.Timestamp(settings["end"])
    candidate_out = Path(paths.get("usgs_streamgage_candidates_geojson") or audit_resolve(paths, spec.get("candidate_output", "data/sources/usgs/streamgage_candidates.geojson")))
    records_out = Path(paths.get("usgs_streamflow_records_csv") or audit_resolve(paths, (spec.get("streamflow_records") or {}).get("output", "data/sources/usgs/streamflow_records.csv")))
    if skip_existing and candidate_out.exists() and audit_covers(paths, "usgs", "streamgages", start, end):
        return AuditArtifact("usgs", "streamgages", start, end, {"candidate_geojson": candidate_out, "streamflow_records_csv": records_out}, {"reused": True})
    ...
    if rec_cfg.get("collect", False):
        site_nos = rec_cfg.get("site_nos") or _reviewed_site_nos(audit_resolve(paths, spec.get("reviewed_network", "data/sources/usgs/streamgage_network.geojson"))) or sites["site_no"].astype(str).tolist()
    ...
    artifact = AuditArtifact("usgs", "streamgages", start, end, {"candidate_geojson": candidate_out, "streamflow_records_csv": records_out}, {"candidate_count": len(sites), "record_count": len(records), "parameter_cd": _parameter(spec)})
    audit_write_artifact(paths, artifact)
    return artifact


def fetch_site_records(spec: dict, paths: dict) -> list[dict]:
    params = _site_params(spec, paths)
    response = requests.get((spec.get("discovery") or {}).get("url", USGS_SITE_SERVICE_URL), params=params, timeout=int((spec.get("discovery") or {}).get("request_timeout_seconds", 60)))
    response.raise_for_status()
    return _read_rdb(response.text)
...
def fetch_discharge_records(spec: dict, site_no: str, start, end) -> list[dict]:
    ...
    response = requests.get(rec.get("url") or (USGS_INSTANTANEOUS_VALUES_URL if service == "iv" else USGS_DAILY_VALUES_URL), params=params, timeout=int(rec.get("request_timeout_seconds", 60)))
    response.raise_for_status()
    frame = pd.DataFrame(_read_rdb(response.text)).rename(columns=lambda c: ALIASES.get(str(c), str(c)))
    if frame.empty:
        return []
...
def _site_params(spec: dict, paths: dict) -> dict:
    ...
    bbox = d.get("bbox") or _bbox_from_search_geometry(d.get("search_geometry"), paths, buffer_km=d.get("hydrologic_buffer_km"))
    if bbox:
