...
class FloodDepthFragilityCurve:
    """CSV-backed SciPy lognormal CDF parameters imported from ERAD."""
    ...
    def failure_probability(self, depth_m: float | None) -> float:
        if depth_m is None:
            return 0.0
        depth = float(depth_m)
        if not math.isfinite(depth) or depth <= self.loc_m:
            return 0.0
        if self.distribution != "lognorm":
            raise ValueError(f"Unsupported fragility distribution: {self.distribution}")
        z = math.log((depth - self.loc_m) / self.scale_m) / self.shape
...
def load_flood_depth_curves(path: str | Path) -> dict[str, FloodDepthFragilityCurve]:
    """Load reviewed ERAD flood-depth CSV curves without native ERAD imports."""
...
def erad_asset_type(local_asset_type: str, mapping: Mapping[str, str]) -> str:
...
def csv_failure_probability(
    local_asset_type: str,
    depth_m: float | None,
    *,
    curves: Mapping[str, FloodDepthFragilityCurve],
    mapping: Mapping[str, str],
) -> float:
    """CSV-backed flood failure probability for notebook compatibility skins."""

    curve_key = erad_asset_type(local_asset_type, mapping)
    return curves[curve_key].failure_probability(depth_m)
