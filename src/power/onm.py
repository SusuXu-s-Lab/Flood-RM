...
def build_event_window_bundle(*, event_start: datetime, horizon_hours: int, load_profiles: pd.DataFrame, blocks: pd.DataFrame, sandbox_id: str, uncertainty_band: float = 0.20, event_id: str = "preview_event", mc_draw: int = 0) -> dict[str, Any]:
    ...
    for row in load_profiles.to_dict("records"):
        ...
        feeder_id = "" if pd.isna(row.get("feeder_id")) else str(row.get("feeder_id") or "")
        demand = {"load_asset_id": load_asset_id, "loadshape_id": str(row["loadshape_id"]), "block_id": block_id, "feeder_id": feeder_id, "customer_class": str(row.get("customer_class", "")), "peak_kw": float(row["peak_kw"]), "values": window.values, "start_hour_of_year": window.start_hour_of_year, "wrapped_across_year_boundary": window.wrapped_across_year_boundary}
    ...
    uncertainty_bands = build_load_uncertainty_bounds(nominal, event_id=event_id, mc_draw=mc_draw, band_fraction=uncertainty_band)
    block_summary = _block_demand_summary(nodal)
    return {"event_start": start.isoformat(), "event_end": (start + timedelta(hours=horizon_hours)).isoformat(), "horizon_hours": horizon_hours, "timestep_count": horizon_hours, "load_profile_count": len(load_profiles), "block_count": len(block_summary), "uncertainty_band": uncertainty_band, "uncertainty_row_count": len(uncertainty_bands), "weather_years": sorted(years), "nodal_demand": nodal, "uncertainty_bands": uncertainty_bands, "block_demand_summary": block_summary}
...
def build_asset_to_dss_element_map(*, assets: pd.DataFrame, controllable_switches: pd.DataFrame | None = None) -> dict[str, str]:
    classes = {"lines": "line", "line": "line", "switches": "line", "transformers": "transformer", "loads": "load", "load_buses": "load", "generators": "generator", "storage": "storage", "solar": "pvsystem"}
    out: dict[str, str] = {}
