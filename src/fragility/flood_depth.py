"""Evaluate ERAD-derived flood-depth fragility curves for grid assets."""
...
def load_asset_type_mapping(path: str | Path) -> dict[str, str]:
    """Load local asset type to ERAD asset type mapping."""
...
def erad_asset_type(local_asset_type: str, mapping: dict[str, str]) -> str:
    """Map a local asset type name to the ERAD curve asset type."""
...
def failure_probability(
    ...
    *,
    ...
    """Return ERAD-derived flood failure probability for a local asset."""
    curves = curves or load_flood_depth_curves()
    curve_key = erad_asset_type(local_asset_type, mapping=mapping)
