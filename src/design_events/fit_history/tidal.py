"""Tide / non-tidal-residual split of the coastal water-level record (ADR-0011, Fix 2).
...
surge-rainfall dependence the copula is meant to capture. Following Maduwantha et al. (2026)
we split CORA total water level into a slow mean sea level, the astronomical tide (utide
harmonic analysis), and the non-tidal residual (NTR = storm surge). The copula axis and the
...
def coastal_components(waterlevel, *, latitude, msl_window="30D", msl_min_periods=200):
    ...
    seasonal sea-level cycle (paper), which also resolves the Stage-2-vs-copula detrend
