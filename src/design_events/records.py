...
# Source-record schemas (see locations/*/data/sources/...). AORC SST stores per-storm
# basin-mean depths (sparse storm-event series), while NWM soil moisture and USGS
# discharge may carry multiple rows per timestamp and are aggregated to one value per time.
def record_specs_from_config(config):
    """Per-driver record specs declared in runtime config or ``event_catalog.dependence``.

    Location specifics (which file, which columns) live in each Location Configuration,
    not in code, so the same notebook process works across Study Locations.
    """
    try:
        return _record_specs(config)
    except ValueError as exc:
        raise ValueError("config event_catalog.dependence.driver_records is required for copula_joint") from exc


def cooccurrence_params_from_config(config):
    """Two-sided POT co-occurrence sampling parameters from config.

    ``target_rate_per_year`` calibrates each conditioning threshold to a target exceedance
    rate instead of a fixed quantile; ``condition_on`` restricts conditioning to the
    extreme forcing drivers when an antecedent state is paired but never conditioned.
    """
    cooccurrence = _dependence(config).get("cooccurrence", {}) or {}
    params = {
        "threshold_quantiles": cooccurrence.get(
            "threshold_quantiles",
            cooccurrence.get("threshold_quantile", 0.95),
        ),
    ...
    condition_on = _dependence(config).get("condition_on") or cooccurrence.get("condition_on")
    if condition_on:
        params["condition_on"] = list(condition_on)
    if cooccurrence.get("dropna") is not None:
        params["dropna"] = bool(cooccurrence["dropna"])
    return params
...
def assemble_paired_observations_from_config(config, *, location_root=None, sites=None, **overrides):
    """Config-driven paired-observation assembly across Study Locations."""
    ...
    return assemble_paired_observations(
        record_specs_from_config(config),
        driver_vector,
...
def build_member_libraries(config, *, location_root=None):
    """Build realization member libraries for each configured driver.

    ``event_catalog.dependence.member_libraries[driver].from`` selects either
    ``member_table`` (a curated forcing-member CSV with field pointers) or ``records``
    (built per timestamp or per declustered peak from the driver records).
    """
    dependence = _dependence(config)
        ...
        if source == "member_table":
            ...
            if decluster_window:
                ...
                if spec.get("target_rate_per_year") is not None:
                    ...
                    peaks = declustered_pot_peaks(
                        series,
                        threshold_quantile=float(spec.get("threshold_quantile", 0.98)),
                        min_separation_hours=float(decluster_window),
                    )
...
__all__ = [
    ...
    "record_specs_from_config", "cooccurrence_params_from_config",
    "assemble_paired_observations", "assemble_paired_observations_from_config", "build_member_libraries",
