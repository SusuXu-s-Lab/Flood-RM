...
from .nlr_distribution_case.core import CasePaths
...
def build_power_runtime(definition: LocationDefinition) -> PowerRuntime:
    ...
    power_grid = _location_path(root, grid.get("power_grid_root", "data/power_grid"))
    paths = CasePaths(
        root=root,
        ...
        reports=_location_path(root, grid.get("reports", "outputs/validation_audit")),
...
def _grid_path(root: Path, grid: dict[str, Any], key: str, default: str | Path) -> Path:
    return _location_path(root, grid.get(key, default))
