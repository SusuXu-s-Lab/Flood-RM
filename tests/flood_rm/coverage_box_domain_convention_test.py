...
def test_greensboro_region_setup_writes_native_river_inflow_gauge_points_for_selected_rural_domain():
    location_root, _ = _location_config("greensboro")

    rural = gpd.read_file(location_root / "data/wflow/domain_set_gauges/greensboro_rural_sfincs_gauges.geojson")

    assert len(rural) == 3
    assert set(rural["wflow_submodel_id"]) == {"greensboro_rural"}
    assert rural["name"].str.startswith("greensboro_rural_inflow_").all()
    assert set(rural["gauge_location_source"]) == {"sfincs_native_river_inflow"}
...
def test_austin_uses_one_selected_sfincs_box_and_one_enclosing_wflow_watershed(monkeypatch, tmp_path):
    ...
    assert runtime_config["wflow"]["domain_set"]["allow_multiple_submodels"] is True
    assert runtime_config["sfincs_domain_set"]["allow_multiple_domains"] is True
    assert runtime_config["sfincs_domain_set"]["include_domain_ids"] == ["austin_p5u"]
    assert runtime_config["aoi"]["source"] == "data/smart_ds/2016"
    assert wflow_plan.status == "ready"
    assert [submodel["wflow_submodel_id"] for submodel in wflow_plan.submodels] == ["austin_p5u"]
    assert sfincs_plan.status == "ready"
    assert sfincs_plan.domain_count == 1
    assert sfincs_plan.domains[0]["sfincs_domain_id"] == "austin_p5u"
    assert sfincs_plan.domains[0]["wflow_submodel_ids"] == ["austin_p5u"]


def test_austin_region_setup_artifacts_keep_selected_p5u_domain():
    location_root, _ = _location_config("austin")
    ...
    assert set(coverage_boxes["subregion_id"]) == {"austin_p5u"}
    assert len(watersheds) == 1
    assert set(watersheds["wflow_submodel_id"]) == {"austin_p5u"}
