...
def test_greensboro_flood_notebooks_have_stage_contracts_and_compile():
    ...
    for notebook_path in notebooks:
        ...
        if notebook_path.parent.name == "04":
            assert "load_coupled_runtime" in text, notebook_path
            assert 'location_root = Path("../..").resolve()' in text, notebook_path
        else:
            assert "Greensboro" in text or "greensboro" in text, notebook_path
...
def test_greensboro_notebooks_include_inland_wflow_components():
    expected_sections = {
        "01_region_setup.ipynb": [
            ...
            "## SFINCS Coverage Boxes and Hydrologic Preflight",
            "## SFINCS Domain Set and Coverage Plot",
            "## Static Source Plan",
            "## SFINCS and Wflow Terrain/Landcover Inputs",
            "## SFINCS SSURGO and Infiltration Inputs",
            "## Wflow/SFINCS Domain Planning Inputs",
            "## Region Setup QA",
        ],
        "02_collect_sources.ipynb": [
...
def test_greensboro_notebooks_reference_usgs_wflow_and_sfincs_handoff():
    ...
    assert "force_sfincs_domain_build = False" in combined_text
    ...
    assert "write_inland_sfincs_handoff_locations(" in build_text
    assert "from hydromt_sfincs import DATADIR, SfincsModel" in combined_text
    assert "sf = SfincsModel(" in combined_text
    assert "\"grid.create_from_region\"" in combined_text
    assert "\"elevation.create\"" in combined_text
    assert "\"mask.create_active\"" in combined_text
    assert "\"roughness_list\"" in combined_text
    assert "\"subgrid.create\"" in combined_text
