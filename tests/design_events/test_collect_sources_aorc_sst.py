...
def test_open_aorc_year_uses_default_noaa_zarr_pattern(monkeypatch):
    captured = {}
    ...
    ds = _open_aorc_year(2022, {})

    assert captured["path"] == DEFAULT_AORC_ZARR_YEAR_PATTERN.format(year=2022)
    assert captured["kwargs"]["engine"] == "zarr"
    assert captured["kwargs"]["consolidated"] is True
    assert "APCP_surface" in ds


def test_study_footprint_uses_smart_ds_evaluation_footprint(tmp_path):
