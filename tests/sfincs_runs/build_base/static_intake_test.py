...
def test_build_region_setup_uses_aoi_study_area_without_grid_footprint(tmp_path):
    location_root = tmp_path / "locations/greensboro"
    study_area = location_root / "data/static/aoi/study_area.geojson"
    study_area.parent.mkdir(parents=True)
    gpd.GeoDataFrame(
        {"name": ["study_area"]},
    ...
    setup = build_region_setup(
        config,
        {"repo_root": tmp_path, "location_name": "greensboro", "location_root": location_root},
    )

    assert setup.study_area_path == study_area
    assert setup.bbox_wgs84 == pytest.approx((-80.01, 35.99, -79.89, 36.11))
    assert setup.dem_raw == location_root / "data/static/raw/topo/dem.tif"
    assert setup.landcover_raw == location_root / "data/static/raw/landcover/landcover.tif"
    assert setup.dem_output == location_root / "data/static/processed/dem_region_setup.tif"
    assert setup.landcover_output == location_root / "data/static/processed/landcover_region_setup.tif"
    assert setup.bbox_output == location_root / "data/static/aoi/bbox.geojson"
    assert setup.ssurgo_output == location_root / "data/static/soils/ssurgo_mapunitpoly.gpkg"
    assert setup.ssurgo_attributes_output == location_root / "data/static/soils/ssurgo_mapunit_attributes.csv"
    assert setup.ssurgo_hsg_output == location_root / "data/static/soils/hsg_greensboro.tif"
    assert setup.ssurgo_ksat_output == location_root / "data/static/soils/ksat_mmhr_greensboro.tif"
